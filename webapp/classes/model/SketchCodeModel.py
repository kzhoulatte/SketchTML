...
from keras.layers import Embedding, MaxPooling2D, GRU, TimeDistributed, RepeatVector, LSTM, concatenate , Input, Reshape, Dense
...
weights = np.array([1,1,1,1,1,1,1,1,1,3,1,1,3,3,3,1,1,1,1,1,1,1,1])
...
class SketchCodeModel():
    ...
    def __init__(self, model_output_path, model_json_file=None, model_weights_file=None):
        ...
        if model_json_file is not None and model_weights_file is not None:
            ...
            self.model.compile(loss=[weighted_categorical_crossentropy(weights)], optimizer=optimizer)
            #self.model.compile(loss=[focal_loss(gamma=2,alpha=0.6)], optimizer=optimizer)
    ...
    def create_model(self):
        ...
        # Customed Image encoder by KZ
        image_model = Sequential()
        image_model.add(Conv2D(8, (3,3), padding='valid', activation='relu', input_shape=(256, 256, 3,)))
